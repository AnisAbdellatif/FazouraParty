...
void main() {
  ...
  test(
    'refuses a file heavier than the server would take, before parsing it',
    () {
      // 32 MB is the server's cap on a package; anything past it is not read at all.
      expect(
        () => QuizArchive.decode(Uint8List(32 * 1024 * 1024 + 1)),
        throwsA(
          isA<FormatException>().having(
            (error) => error.message,
            'message',
            'Quiz archive is too large.',
          ),
        ),
      );
    },
